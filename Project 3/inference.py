...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        all_beliefs = DiscreteDistribution(util.Counter())

        for pos in self.allPositions:
            # update beliefs based on the distance observation and pacman's position
            x = self.getObservationProb(noisy_distance, pacman_position, pos, self.getJailPosition())
            all_beliefs[pos] = x * self.beliefs[pos]

        all_beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        all_beliefs = DiscreteDistribution(util.Counter())
        for pos in self.allPositions:
            new_pos = self.getPositionDistribution(gameState, pos)
            # predict beliefs for the next time step
            for new_position, prob in new_pos.items():
                all_beliefs[new_position] += prob * self.beliefs[pos]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for x in range(self.numParticles):
            pos = self.legalPositions[x % len(self.legalPositions)]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        pacman_position = gameState.getPacmanPosition()
        all_beliefs = DiscreteDistribution(util.Counter())
        old_dist = self.getBeliefDistribution()

        for pos in self.legalPositions:
            x = self.getObservationProb(noisy_distance, pacman_position, pos, self.getJailPosition())
            all_beliefs[pos] = x * old_dist[pos]
        
        # handles edge case when all particles receive zero weight
        if all_beliefs.total() == 0:
            self.initializeUniformly(gameState)
        else:
            final_particles = []
            for i in range(self.numParticles):
                final_particles.append(all_beliefs.sample())
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.particles:
            new_pos = self.getPositionDistribution(gameState, pos)
            new_states.append(new_pos.sample())
    ...
    def getBeliefDistribution(self):
        ...
        state = DiscreteDistribution(util.Counter())
        for particle in self.particles:
            # increase count of particle
            state[particle] += 1
        state.normalize()
        return state
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        random_particles = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(random_particles)

        for i in range(self.numParticles):
            self.particles.append(random_particles[i % len(random_particles)])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            prob = 1
            for i in range(self.numGhosts):
                prob *= self.getObservationProb(noisy_distances[i], pacman_position, particle[i], self.getJailPosition(i))
            all_beliefs[particle] +=  prob
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                new_pos = self.getPositionDistribution(gameState, oldParticle, i, self.ghostAgents[i])
                newParticle[i] = new_pos.sample()
