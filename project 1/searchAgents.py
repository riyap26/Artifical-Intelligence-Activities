...
class CornersProblem(search.SearchProblem):
    ...
    def isGoalState(self, state):
        ...
        "*** YOUR CODE HERE ***"
        is_corner_visited = state[1]
        for corner in is_corner_visited:
            if corner == False:
                return False
        return True
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.EAST, Directions.WEST, Directions.NORTH, Directions.SOUTH]:
            ...
            new_corners_visited = list(state[1])

            if not hitsWall:
                for index, corner in enumerate(self.corners):
                    if corner == (nextx, nexty):
                        new_corners_visited[index] = True
                successors.append((((nextx, nexty), tuple(new_corners_visited)) , action, 1))
...
def cornersHeuristic(state, problem):
    ...
    current_point = state[0] # Current coordinates
    
    largest_distance = 0
    # go through all the unvisited corners
    # get mandistance
    # return largest mandistance
    for index, corner in enumerate(state[1]):
        if corner == False:
            distance_to_corner = util.manhattanDistance(corners[index], current_point)
            largest_distance = max(largest_distance, distance_to_corner)
...
def foodHeuristic(state, problem):
    ...
    # Get separation distance from nearest food to furthest food
    max_distance = 0
    for food in foodList:
        distance = util.manhattanDistance(min_food, food)
