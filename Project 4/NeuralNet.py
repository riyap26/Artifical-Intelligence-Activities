...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        # add 1 to start of inActs for bias term
        bias_inActs = [1] + inActs
        answer = []
        for i in range(len(bias_inActs)):
            answer.append(bias_inActs[i] * self.weights[i])
        return self.sigmoid(sum(answer))
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        biased_inActs = [1] + inActs
        for i in range(len(self.weights)):
            self.weights[i] += biased_inActs[i] * alpha * delta
            totalModification += abs(biased_inActs[i] * alpha * delta)
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        # first index has to be input list
        answer = [inActs]
        before = inActs
        ...
        for layer in self.layers:
            ...
            for perceptron in layer:
                perceptrons_act.append(perceptron.sigmoidActivation(before))
            answer.append(perceptrons_act)
            before = perceptrons_act
        return answer
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    for i in range(len(nextLayer)):
                        error_next +=  deltas[0][i] * nextLayer[i].weights[neuronNum + 1]
                    delta = gPrime * error_next
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    for inp, out in examplesTest:
        outcome_result = nnet.feedForward(inp)[-1]
        is_true = True
        for i in range(len(outcome_result)):
            if round(outcome_result[i]) != out[i]:
                is_true = False
        if is_true == False:
            testError += 1
        else:
